...
#include "sw/device/lib/crypto/impl/sm2/ecdh_p256.h"
...
otbn_app_t kOtbnAppEcdh;

void initialize_otbn_app_ecdh(void){
  kOtbnAppEcdh.imem_start = OTBN_SYMBOL_PTR(p256_ecdh, _imem_start);
  kOtbnAppEcdh.imem_end = OTBN_SYMBOL_PTR(p256_ecdh, _imem_end);
  kOtbnAppEcdh.dmem_data_start = OTBN_SYMBOL_PTR(p256_ecdh, _dmem_data_start);
  kOtbnAppEcdh.dmem_data_end = OTBN_SYMBOL_PTR(p256_ecdh, _dmem_data_end);
  kOtbnAppEcdh.dmem_data_start_addr = OTBN_ADDR_T_INIT(p256_ecdh, _dmem_data_start);
}

const uint32_t *kOtbnVarEcdhMode;
const uint32_t *kOtbnVarEcdhX;
const uint32_t *kOtbnVarEcdhY;
const uint32_t *kOtbnVarEcdhD0;
const uint32_t *kOtbnVarEcdhD1;

void initialize_otbn_pointers_ecdh(){
  kOtbnVarEcdhMode = OTBN_SYMBOL_ADDR(p256_ecdh, mode);
  kOtbnVarEcdhX = OTBN_SYMBOL_ADDR(p256_ecdh, x);
  kOtbnVarEcdhY = OTBN_SYMBOL_ADDR(p256_ecdh, y);
  kOtbnVarEcdhD0 = OTBN_SYMBOL_ADDR(p256_ecdh, d0);
  kOtbnVarEcdhD1 = OTBN_SYMBOL_ADDR(p256_ecdh, d1);
}
