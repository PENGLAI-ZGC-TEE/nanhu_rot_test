...
void PUF_RNG_ON(uint64_t base_addr) {
...
void PUF_OFF(uint64_t base_addr) {
    // disable PUF 
    uint32_t val = abs_mmio_read32(base_addr + PUF_CTRL_SIGNALS_REG_OFFSET); 
    val &= ~(1 << PUF_CTRL_SIGNALS_ENABLE_PUF_BIT);
    abs_mmio_write32(base_addr + PUF_CTRL_SIGNALS_REG_OFFSET, val); 
...
void PUF_RNG_OFF(uint64_t base_addr) {
    // set puf to puf mode
    uint32_t val = abs_mmio_read32(base_addr + PUF_CTRL_SIGNALS_REG_OFFSET); 
...
uint8_t checkBit(uint64_t addr, uint32_t bit) {
    return (abs_mmio_read32(addr) & (1u << bit)) != 0;
}

void wait_for_res_valid_BIT(uint64_t base_addr) {
    while (!checkBit(base_addr + PUF_STATE_SIGNALS_REG_OFFSET, PUF_STATE_SIGNALS_RESPONSE_VALID_BIT_BIT)) {

    }
}

void puf_get_res_of_a_cha(uint64_t base_addr, uint32_t challenge_input[], uint32_t response_output[])
{    
    // disable PUF first
    uint32_t val = abs_mmio_read32(base_addr + PUF_CTRL_SIGNALS_REG_OFFSET); 
    val &= ~(1 << PUF_CTRL_SIGNALS_ENABLE_PUF_BIT);
