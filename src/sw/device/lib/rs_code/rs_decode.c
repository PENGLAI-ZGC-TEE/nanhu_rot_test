...
uint8_t checkBit_rs_decode(uint64_t addr, uint32_t bit) {
    return (abs_mmio_read32(addr) & (1u << bit)) != 0;
}

// is rs decode complete
int wait_for_rs_decode_valid_BIT(uint64_t base_addr) {
    int i;
    for (i = 0; i < 200; i++) {
...
void rs_decode_function(uint64_t base_addr, uint32_t encoded_data_err_input[],uint32_t error_pos_output[])
{
