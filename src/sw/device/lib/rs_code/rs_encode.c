...
uint8_t checkBit_rs_encode(uint64_t addr, uint32_t bit) {
    return (abs_mmio_read32(addr) & (1u << bit)) != 0;
}

// is rs encode complete
void wait_for_rs_encode_valid_BIT(uint64_t base_addr) {
...
void rs_encode_function(uint64_t base_addr, uint32_t data_input[], uint32_t encoded_output[])
{
